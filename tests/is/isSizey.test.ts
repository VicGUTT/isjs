...
import isSizey from '../../src/isSizey.js';
...
describe('is/isSizey', () => {
    it(`❌ nil !== ${KEY}`, () => {
        values.nil.forEach((value) => {
            expect(isSizey(value)).toEqual(false);
        });
    });
    ...
    it(`➖ DOM elements !== || === ${KEY}`, () => {
        values.elements.forEach((value) => {
            expect(isSizey(value)).toEqual(value instanceof HTMLInputElement);
        });
    });
    ...
    it(`➖ object instances !== || === ${KEY}`, () => {
        values.instances.forEach((value) => {
            expect(isSizey(value)).toEqual(typeof value?.size === 'number');
        });
    });
